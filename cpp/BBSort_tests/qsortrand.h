#ifndef BBSORT_SOLUTION_QSORTRAND_H
#define BBSORT_SOLUTION_QSORTRAND_H
...
template<typename RandomAccessIterator,
        typename Order>
void quicksortrand(RandomAccessIterator first, RandomAccessIterator last, Order order)
{
    if (first != last && first+1 != last)
    {
        ...
        quicksortrand(first, split1, order);
        quicksortrand(split2, last, order);
    }
}

template<typename RandomAccessIterator>
void quicksortrand(RandomAccessIterator first, RandomAccessIterator last)
{
    quicksortrand(first, last, std::less<typename std::iterator_traits<RandomAccessIterator>::value_type>());
}
